...
export function htmlToText(html, extensions) {
    ...
    let doubleNewlineTags = [
        'p', 'h[1-6]', 'dl', 'dt', 'dd', 'ol', 'ul', 'dir', 'address', 'blockquote', 'center', 'hr', 'pre',
        'form', 'textarea', 'table'
    ];

    let singleNewlineTags = [
        'li', 'del', 'ins', 'fieldset', 'legend', 'div', 'tr', 'th', 'caption', 'thead', 'tbody', 'tfoot'
    ];

    for (let i = 0; i < doubleNewlineTags.length; i++) {
        let r = new RegExp('</?\\s*' + doubleNewlineTags[i] + '[^>]*>', 'ig');

        text = text.replace(r, '\n\n');
    }

    for (let i = 0; i < singleNewlineTags.length; i++) {
        let r = new RegExp('<\\s*' + singleNewlineTags[i] + '[^>]*>', 'ig');
